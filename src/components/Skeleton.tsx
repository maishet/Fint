...
export function SkeletonGroup({ children, label }: SkeletonGroupProps) {
  ...
  useEffect(() => {
    ...
    const animation = Animated.loop(
      Animated.sequence([
        Animated.timing(opacity, { duration: 750, toValue: 0.9, useNativeDriver: true }),
        Animated.timing(opacity, { duration: 750, toValue: 0.5, useNativeDriver: true }),
      ]),
    )
    animation.start()
  ...
  return (
    <Animated.View accessible accessibilityLabel={label} accessibilityRole="progressbar" style={{ opacity }}>
      <YStack gap="$4" accessibilityElementsHidden importantForAccessibility="no-hide-descendants">
        {children}
      </YStack>
    </Animated.View>
  )
...
export function SkeletonBlock({ bg = '$color4', height = 14, rounded = '$4', width = '100%', ...props }: YStackProps) {
  return <YStack bg={bg} height={height} rounded={rounded} width={width} {...props} />
}
...
export function SkeletonList({ grouped = false, rows = 3 }: SkeletonListProps) {
  const content = Array.from({ length: rows }, (_, index) => (
    <YStack key={index}>
      ...
      <XStack items="center" gap="$3" minH={64} px={grouped ? '$4' : undefined} py={grouped ? '$2' : undefined}>
    ...
    <YStack gap="$3">
      ...
      <FintCard gap="$3" minH={height}>
        ...
        <XStack flex={1} items="flex-end" justify="space-around" gap="$3">
          {[46, 82, 64, 108].map((barHeight, index) => <SkeletonBlock key={index} height={barHeight} rounded="$3" width={26} />)}
        </XStack>
      </FintCard>
    </YStack>
  )
        ...
        <XStack key={row} gap="$2">
          {[0, 1].map((column) => (
            <FintCard key={column} flex={1} gap="$2" p="$3" minH={92}>
              <XStack items="center" gap="$2"><SkeletonBlock height={30} rounded="$8" width={30} /><SkeletonBlock height={10} width="52%" /></XStack>
    ...
    <FintCard gap="$3">
      ...
      {Array.from({ length: rows }, (_, index) => (
        <XStack key={index} items="center" gap="$3">
          ...
          <YStack flex={1} gap="$2"><SkeletonBlock height={13} width="58%" /><SkeletonBlock height={9} width="40%" /></YStack>
