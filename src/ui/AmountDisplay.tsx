...
const HEIGHT = 72;
/**
 * Geist Mono es monoespaciada: todo carácter avanza lo mismo, así cada pieza se ubica con cálculo. El avance
 * se mide una vez en el dispositivo (diez ceros a 40px, que caben sin partirse); mientras tanto, 0.6em.
 */
const ADVANCE = 0.6;
const PROBE_SIZE = 40;
...
const LINE = 1.0625;
/** Distancia de la parte de arriba de la caja a la línea base, en proporción al tamaño (Geist Mono con `LINE`). */
const BASELINE = 0.886;
/** El espacio fino de miles ocupa menos que un dígito. */
const THIN_UNITS = 0.22;
...
function prefixWidth(size: number, signOn: number, symbolLength: number, advance: number) {
  "worklet";
  const m = size * MINOR;
  return signOn * (m * advance + 6) + symbolLength * m * advance + 8;
}
...
function rowStart(g: Geometry) {
  ...
  const total = prefixWidth(s, g.signOn.value, g.symbolLength, a) + g.units.value * digitAdvance(s, a) + (g.caret ? 7 : 0);
 ...
 *
 * Cada pieza se ubica con cálculo (la fuente es monoespaciada) y se anima sola:
...
export function AmountDisplay({ input, currency = "PEN", kind = "expense", active = true }: AmountDisplayProps) {
  ...
  // dígito conserva su clave (y su animación) aunque cambie la agrupación.
  ...
  const fits = (s: number) =>
    prefixWidth(s, sign ? 1 : 0, symbol.length, advanceEm) + glyphs.units * digitAdvance(s, advanceEm) + 7 + 12 <= measured;
  ...
  const geometry: Geometry = { width, advance, size, units, signOn, symbolLength: symbol.length, caret: active };
  ...
  const caretStyle = useAnimatedStyle(() => {
    ...
    return {
      ...
      left:
        rowStart(geometry) +
        prefixWidth(s, signOn.value, geometry.symbolLength, advance.value) +
  ...
  return (
    ...
    >
      {/* Sonda invisible: mide el avance real de Geist Mono en este dispositivo. */}
      <Text
        style={{ position: "absolute", opacity: 0, fontFamily: fontFace.mono[500], fontSize: PROBE_SIZE, includeFontPadding: false }}
        onLayout={(e: LayoutChangeEvent) => {
      ...
      {measured > 0 ? (
        <>
          <Animated.Text style={[{ position: "absolute", fontFamily: fontFace.mono[500], color: signColor, includeFontPadding: false }, signStyle]}>
            {sign || MINUS}
          </Animated.Text>
          <Animated.Text
            style={[{ position: "absolute", fontFamily: fontFace.mono[500], color: theme.inkFaint.val, letterSpacing: -1, includeFontPadding: false }, symbolStyle]}
          >
            {symbol}
          </Animated.Text>
          {glyphs.list.map((g) => (
...
function Glyph({
  ...
  const style = useAnimatedStyle(() => {
    ...
    return {
      ...
      left:
        rowStart(geometry) +
        prefixWidth(s, geometry.signOn.value, geometry.symbolLength, geometry.advance.value) +
  ...
  return (
    <Animated.Text
      ...
      style={[{ position: "absolute", fontFamily: fontFace.mono[500], fontVariant: ["tabular-nums"], color, includeFontPadding: false }, style]}
    >
      {ch}
    </Animated.Text>
  );
