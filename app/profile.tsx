...
export default function ProfileScreen() {
  ...
  const save = async () => {
    setServerError(null)
    const schema = z.object({ displayName: z.string().trim().min(2, t('validation.profileName', { defaultValue: t('profile.invalid') })).max(80, t('validation.profileName', { defaultValue: t('profile.invalid') })) })
    ...
    <Screen>
      <FintCard bg={isDark ? '#0B3046' : '#0F5D73'} borderColor={isDark ? '#1B5067' : '#28788C'} gap="$4" items="center" p="$5" overflow="hidden">
        ...
        <YStack items="center" gap="$1">
          ...
          <Paragraph color="#B9D7E1" fontSize="$2" text="center">{t('profile.subtitle')}</Paragraph>
        </YStack>
      </FintCard>
      ...
      <FintCard gap="$3">
        <XStack items="center" gap="$3">
          ...
          <Paragraph color="$primary" fontFamily="$heading" fontSize="$5" fontWeight="800">{t('profile.section')}</Paragraph>
        </XStack>

        <ProfileInputRow error={validation.errors.displayName} icon={<UserRound size={22} color="$primary" />} label={t('profile.name')} onChangeText={(value) => { setDisplayName(value); validation.clearError('displayName') }} placeholder={t('profile.namePlaceholder')} value={displayName} />
        <ReadOnlyRow badge={t('profile.notEditable')} icon={<Mail size={22} color="$primary" />} label={t('profile.email')} value={session?.user.email ?? '-'} />
      ...
      <FintCard gap="$3" bg="$accent1" borderColor="$accent4">
        <XStack gap="$3" items="center">
          ...
          <Paragraph color="$color10" flex={1} fontSize="$3">{t('profile.auth')}</Paragraph>
        </XStack>
      </FintCard>

      <YStack gap="$3">
        <FintButton width="100%" minH={56} disabled={isSaving || isSigningOut} icon={isSaving ? <Spinner size="small" color="$primaryForeground" /> : <Save size={19} color="$primaryForeground" />} onPress={() => { void save() }}>{isSaving ? t('profile.saving') : t('profile.save')}</FintButton>
        <FintButton width="100%" minH={54} variant="outlined" disabled={isSaving || isSigningOut} color="$primary" borderColor="$primary" icon={isSigningOut ? <Spinner size="small" color="$primary" /> : <LogOut size={20} color="$primary" />} onPress={() => { void endSession() }}>{t('profile.signOut')}</FintButton>
      </YStack>
    </Screen>
  )
}
