...
import { FintButton, FintCard, FintConfirmDialog, FintFormField, FintInput } from '../src/ui'
  ...
  return (
    <>
      ...
      {saveErrorMessage ? <Paragraph color="$red10">{saveErrorMessage}</Paragraph> : null}
       <FintButton variant="outlined" color="$red10" borderColor="$red6" disabled={pending} icon={<Trash2 size={16} />} onPress={() => setDisconnectOpen(true)}>{t('gmail.disconnect')}</FintButton>
      </FintCard>
      <FintConfirmDialog open={disconnectOpen} isPending={deleteMutation.isPending} title={t('gmail.disconnect')} description={t('gmail.disconnectConfirm')} cancelLabel={t('actions.cancel')} confirmLabel={t('gmail.disconnect')} destructive icon={<Trash2 size={17} color="white" />} onCancel={() => setDisconnectOpen(false)} onConfirm={() => deleteMutation.mutate()} />
    </>
  )
}
