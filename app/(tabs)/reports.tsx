...
const copy = {
  es: {
    title: 'Cierre financiero', subtitle: 'Una lectura completa y ordenada de tu periodo.', closing: 'Reporte de cierre', filters: 'Configura tu cierre', period: 'Periodo', account: 'Cuenta', currency: 'Moneda', allAccounts: 'Todas las cuentas', currentMonth: 'Mes actual', previousMonth: 'Mes anterior', last3Months: 'Últimos 3 meses', last6Months: 'Últimos 6 meses', updated: 'Generado', mixed: 'Tienes varias monedas. Este cierre muestra únicamente la moneda seleccionada.', loading: 'Preparando tu cierre financiero...', error: 'No pudimos generar el cierre financiero.', empty: 'No hay movimientos confirmados para este periodo.', exportTitle: 'Descargar cierre', exportPdf: 'Documento PDF', exportCsv: 'Archivo CSV', exporting: 'Preparando...', exported: 'Reporte listo', exportError: 'No pudimos exportar el reporte.', executiveSummary: 'Resumen ejecutivo', financialStatus: 'Estado financiero', income: 'Ingresos', expenses: 'Gastos', net: 'Resultado neto', savingsRate: 'Tasa de ahorro', transactions: 'Movimientos', previousPeriod: 'Periodo anterior', flow: 'Evolución del periodo', categories: 'Gastos por categoría', accountActivity: 'Actividad por cuenta', currentPosition: 'Posición actual', accounts: 'Cuentas actuales', debts: 'Deudas activas', topTransactions: 'Movimientos destacados', category: 'Categoría', date: 'Fecha', type: 'Tipo', amount: 'Monto', balance: 'Saldo actual', outstanding: 'Pendiente', dueDate: 'Vencimiento', progress: 'Pagado', noData: 'Sin datos para mostrar', currentSnapshotNote: 'Los saldos y deudas corresponden al momento de generar el reporte; la actividad corresponde al periodo seleccionado.', incomeType: 'Ingreso', expenseType: 'Gasto', viewMovements: 'Ver movimientos', comparison: 'vs. periodo anterior', topCategory: 'Mayor categoría de gasto', largestMovement: 'Movimiento más alto', noPrevious: 'Sin comparación previa', statuses: { healthy: 'Saludable', balanced: 'En equilibrio', attention: 'Requiere atención', no_data: 'Sin actividad' }, statusMessages: { healthy: 'Tus ingresos superaron tus gastos. Mantén este margen y considera separar una parte del resultado neto.', balanced: 'Tus ingresos y gastos están muy cerca. Un pequeño ajuste puede mejorar tu capacidad de ahorro.', attention: 'Tus gastos superaron tus ingresos. Revisa primero las categorías y movimientos de mayor impacto.', no_data: 'Aún no hay información suficiente para evaluar este periodo.' },
  },
  en: {
  ...
  pt: {
    title: 'Fechamento financeiro', subtitle: 'Uma leitura completa e organizada do seu período.', closing: 'Relatório de fechamento', filters: 'Configure seu fechamento', period: 'Período', account: 'Conta', currency: 'Moeda', allAccounts: 'Todas as contas', currentMonth: 'Mês atual', previousMonth: 'Mês anterior', last3Months: 'Últimos 3 meses', last6Months: 'Últimos 6 meses', updated: 'Gerado', mixed: 'Você possui várias moedas. Este fechamento mostra apenas a moeda selecionada.', loading: 'Preparando seu fechamento financeiro...', error: 'Não foi possível gerar o fechamento financeiro.', empty: 'Não há movimentações confirmadas neste período.', exportTitle: 'Baixar fechamento', exportPdf: 'Documento PDF', exportCsv: 'Arquivo CSV', exporting: 'Preparando...', exported: 'Relatório pronto', exportError: 'Não foi possível exportar o relatório.', executiveSummary: 'Resumo executivo', financialStatus: 'Estado financeiro', income: 'Receitas', expenses: 'Despesas', net: 'Resultado líquido', savingsRate: 'Taxa de economia', transactions: 'Movimentações', previousPeriod: 'Período anterior', flow: 'Evolução do período', categories: 'Despesas por categoria', accountActivity: 'Atividade por conta', currentPosition: 'Posição atual', accounts: 'Contas atuais', debts: 'Dívidas ativas', topTransactions: 'Movimentações em destaque', category: 'Categoria', date: 'Data', type: 'Tipo', amount: 'Valor', balance: 'Saldo atual', outstanding: 'Pendente', dueDate: 'Vencimento', progress: 'Pago', noData: 'Sem dados para exibir', currentSnapshotNote: 'Os saldos e dívidas correspondem ao momento da geração; a atividade corresponde ao período selecionado.', incomeType: 'Receita', expenseType: 'Despesa', viewMovements: 'Ver movimentações', comparison: 'vs. período anterior', topCategory: 'Maior categoria de despesa', largestMovement: 'Maior movimentação', noPrevious: 'Sem comparação anterior', statuses: { healthy: 'Saudável', balanced: 'Em equilíbrio', attention: 'Requer atenção', no_data: 'Sem atividade' }, statusMessages: { healthy: 'Suas receitas superaram suas despesas. Mantenha essa margem e considere separar parte do resultado líquido.', balanced: 'Suas receitas e despesas estão muito próximas. Um pequeno ajuste pode melhorar sua capacidade de economizar.', attention: 'Suas despesas superaram suas receitas. Revise primeiro as categorias e movimentações de maior impacto.', no_data: 'Ainda não há informações suficientes para avaliar este período.' },
  },
} as const
...
function StatusCard({ report, text }: { report: FinancialReportPeriod; text: ReportText }) {
  ...
  return <FintCard bg={positive ? '$green2' : attention ? '$red2' : '$secondary'} borderColor={positive ? '$green6' : attention ? '$red6' : '$borderColor'} gap="$3"><XStack items="center" gap="$3"><YStack width={42} height={42} rounded="$9" bg={positive ? '$green4' : attention ? '$red4' : '$accent3'} items="center" justify="center"><Target size={21} color={positive ? '$green10' : attention ? '$red10' : '$primary'} /></YStack><YStack flex={1} gap="$1"><Paragraph color="$color10" fontSize={10} fontWeight="900" letterSpacing={0.8} textTransform="uppercase">{text.financialStatus}</Paragraph><Paragraph color="$color12" fontFamily="$heading" fontSize="$5" fontWeight="800">{text.statuses[report.summary.status]}</Paragraph></YStack></XStack><Paragraph color="$color10">{text.statusMessages[report.summary.status]}</Paragraph>{report.highlights.topExpenseCategory || report.highlights.largestTransaction ? <XStack gap="$2"><Highlight label={text.topCategory} value={report.highlights.topExpenseCategory ? getCategoryLabel(report.highlights.topExpenseCategory.name, t) : text.noData} amount={report.highlights.topExpenseCategory ? formatMoney(report.highlights.topExpenseCategory.amount, report.filters.currency) : undefined} /><Highlight label={text.largestMovement} value={report.highlights.largestTransaction ? getCategoryLabel(report.highlights.largestTransaction.category, t) : text.noData} amount={report.highlights.largestTransaction ? formatMoney(report.highlights.largestTransaction.amount, report.filters.currency) : undefined} /></XStack> : null}</FintCard>
}
